...
double simple_monte_carlo1(double expiry, double strike, double spot,
...
int main() {
  ...
  double result =
      simple_monte_carlo1(expiry, strike, spot, vol, r, number_of_paths);

  std::cout << "The price of digital option is " << result << "\n";
...
double simple_monte_carlo1(double expiry, double strike, double spot,
  ...
  for (unsigned long i = 0; i < number_of_paths; i++) {
    ...
    double this_payoff = (this_spot > strike) ? 1.0 : 0.0;
